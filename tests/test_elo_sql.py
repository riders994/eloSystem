...
@pytest.fixture
def patch_read_sql_table(monkeypatch):
    """Make reset_dims()/pull_dims() return empty frames instead of hitting a DB.
    Records the dims that were requested."""
    requested = []

    def fake_read_sql_table(name, con, schema=None, index_col=None):
        requested.append(name)
        return pd.DataFrame()

    monkeypatch.setattr(elo_data.pd, 'read_sql_table', fake_read_sql_table)
    return requested


def make_elosql(monkeypatch, patch_read_sql_table, config):
...
def test_init_with_conn_dict(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    assert obj.conn_dict == VALID_CONN
    assert isinstance(obj.conn, FakeConn)


def test_init_with_conn_uri(monkeypatch, patch_read_sql_table):
    uri = 'postgresql://alice:secret@localhost:5432/mydb'
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_uri': uri})
...
def test_init_no_connection_details_raises(monkeypatch, patch_read_sql_table):
    with pytest.raises(KeyError, match='No connection details supplied'):
        EloSQL({}, connector=FakeConn())


def test_init_default_schema(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    assert obj.schema == 'fantasy_sports'


def test_init_schema_override_from_config(monkeypatch, patch_read_sql_table):
    obj = make_elosql(
        monkeypatch, patch_read_sql_table,
        {'conn_dict': VALID_CONN, 'schema': 'custom_schema'},
    )
    assert obj.schema == 'custom_schema'


def test_init_calls_reset_dims_for_all_elo_dims(monkeypatch, patch_read_sql_table):
    make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    # reset_dims pulls every ELO_DIM as dim_<name>
    assert set(patch_read_sql_table) == {'dim_league', 'dim_manager', 'dim_team'}


def test_init_does_not_call_connect_when_connector_given(monkeypatch, patch_read_sql_table):
    def boom(*a, **k):
...
def test_pull_dim_skips_when_present_and_not_overwrite(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    patch_read_sql_table.clear()

    # 'team' is already cached by reset_dims, so overwrite=False must skip it.
    assert obj._pull_dim('team', overwrite=False) is True
    assert patch_read_sql_table == []


def test_pull_dim_reads_when_overwrite(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    patch_read_sql_table.clear()

    assert obj._pull_dim('team', overwrite=True) is True
    assert patch_read_sql_table == ['dim_team']


def test_pull_dim_reads_when_absent(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    patch_read_sql_table.clear()
    obj.dim_tables.pop('manager')

    assert obj._pull_dim('manager', overwrite=False) is True
    assert patch_read_sql_table == ['dim_manager']
...
def test_elo_publish_prep_sets_is_dynasty_false(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
...
def test_elo_publish_prep_dynasty_calls_set_dynasty_season(monkeypatch, patch_read_sql_table):
    obj = make_elosql(monkeypatch, patch_read_sql_table, {'conn_dict': VALID_CONN})
    _seed_dims_for_publish(obj)
